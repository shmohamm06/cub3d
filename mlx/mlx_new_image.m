...
void    *mlx_new_image(mlx_ptr_t *mlx_ptr, int width, int height)
{
  ...
  newimg->buffer = malloc(UNIQ_bits_per_pixel*width*height);
  bzero(newimg->buffer, UNIQ_bits_per_pixel*width*height);
...
// assume here 32bits_per_pixel little endian

char    *mlx_get_data_addr(mlx_img_list_t *img_ptr, int *bits_per_pixel, int *size_line, int *endian)
{
  *bits_per_pixel = UNIQ_bits_per_pixel*8;
  *size_line = img_ptr->width*UNIQ_bits_per_pixel;
